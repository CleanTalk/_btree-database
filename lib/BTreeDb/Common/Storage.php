...
class Storage extends \ArrayObject {
	...
	/**
	 * Converts array to \BTreeDb\Common\Storage type.
	 * Recursive.
	 *
	 * @param $data_name
	 * @param array $data
	...
	public function convertToStorage( $data_name, $data ){
		...
		return new \BTreeDb\Common\Storage( $data_name, $data );
	}

	/**
	 * Converts \BTreeDb\Common\Storage to array type.
	 * Recursive.
	 *
	 * @param null $data
	 *
	...
	public function convertToArray( $data = null ){
		...
		$tmp = array();
		foreach ( $data as $name => $item ) {
			$tmp[ $name ] = $item instanceof \BTreeDb\Common\Storage
	...
	public function save()
	{
		...
		switch( $this->type ){
			...
			case 'php':
				...
				\BTreeDb\Common\File::inject__variable(
					$filename,
					$this->storage_name,
					$this->convertToArray()
				);
	...
	public function is_empty(){
        $empty = false;
		foreach ( $this as $this1 => $value) {
			$empty = !($empty) ? ! isset( $value ) : $empty;
		}
		return $empty;
	}
