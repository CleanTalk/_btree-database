...
class Helper{
    ...
    static function ip__canonical_to_long($ip_string)
    {
        if ( !empty($ip_string) ) {
            if ( strpos($ip_string, '/') !== false){
                $ip_string_array = explode('/', $ip_string, 2);
                $network = $ip_string_array[0];
                $mask = (int)$ip_string_array[1];
            } else {
                $network = $ip_string;
                $mask = 32;
            }
            if ( self::ip__validate($network) === 'v4' ){
            ...
            $mask = bindec( str_pad( str_repeat( '1', $mask ), 32, 0, STR_PAD_RIGHT ) );
            return array('network' => $network, 'mask' => $mask);
        }
        return false;
    }
